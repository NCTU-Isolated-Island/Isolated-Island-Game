        ...
        public override bool Handle(PlayerFetchDataCode fetchCode, ErrorCode returnCode, string fetchDebugMessage, Dictionary<byte, object> parameters)
        {
            if (base.Handle(fetchCode, returnCode, fetchDebugMessage, parameters))
            {
                try
                {
                    ...
                    Blueprint.ElementInfo[] requirements = SerializationHelper.BlueprintElementInfoArrayDeserialize((byte[])parameters[(byte)FetchAllKnownBlueprintsResponseParameterCode.Requirements]);
                    Blueprint.ElementInfo[] products = SerializationHelper.BlueprintElementInfoArrayDeserialize((byte[])parameters[(byte)FetchAllKnownBlueprintsResponseParameterCode.Products]);
